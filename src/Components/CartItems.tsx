import React from "react";
import { Box, Center, HStack, Pressable, Image, VStack,Text, Button } from "native-base";
import { SwipeListView } from "react-native-swipe-list-view";
import { Products } from "../data";
import { Colors } from "../color";
import { FontAwesome } from '@expo/vector-icons';

const Swiper = () => (
  <SwipeListView
    rightOpenValue={-50}
    previewRowKey='0'
    previewOpenValue={-40}
    previewOpenDelay={3000}
    data={Products.slice(0,3)}
    renderItem=
    {
      (data) => (
        <Pressable>
          <Box ml={6} mb={3}>
            <HStack alignItems="center" bg={Colors.white} shadow={1} rounded={10} overflow="hidden">
              <Center w='25%' bg={Colors.sub_gray}>
                <Image source={{ uri: data.item.image }} alt={data.item.name} w="full" h={24} resizeMode="contain" />
              </Center>
              <VStack w='66%' px={2} space={3}>
                <Text isTruncated color={Colors.black} bold fontSize={10}>
                  {data.item.name}
                </Text>
                <Text bold color={Colors.dark_gray}>${data.item.price}</Text>
              </VStack>
              <Center>
                <Button bg={Colors.green} _pressed={{bg:Colors.green}} _text={{color:Colors.white}}>$</Button>
              </Center>
            </HStack>
          </Box>
        </Pressable>
      )
    }

    renderHiddenItem=
    {() => (
      <Pressable w={50} roundedTopRight={10} roundedBottomRight={10} h='88%' ml="auto" justifyContent="center" bg={Colors.red}>
        <Center alignItems='center' p={2}>
          <FontAwesome name="trash" size={24} color={Colors.white} />
        </Center>
      </Pressable>
    )
    }

    showsVerticalScrollIndicator={false}
  />
)

export default function CartItems() {
  return (
    <Box mr={6}>
      <Swiper />
    </Box>
  );
}
